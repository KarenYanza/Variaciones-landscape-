    ...
    private List<Earthquake> parseEarthquake (String responseString){
        ...
        try {
            ...
            for (int i=0; i<featuresJSONArray.length();i++){
                ...
                String ma= df.format(magnitude);
                String lo= df.format(longitude);
                String la= df.format(latitude);

                Earthquake earthquake=new Earthquake(id,place,Double.parseDouble(ma),time,Double.parseDouble(la),Double.parseDouble(lo));
